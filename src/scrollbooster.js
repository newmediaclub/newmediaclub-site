...
export default class ScrollBooster {
  ...
  setContentPosition(state) {
    if (this.props.scrollMode === "transform") {
      this.props.content.style.transform = `translate(${-state.position
        .x}px, ${-state.position.y}px)`;
    }
  ...
  handleEvents() {
    ...
    this.events.pointerdown = event => {
      ...
      if (
        ...
        formNodes.indexOf(event.target.nodeName.toLowerCase()) > -1
      ) {
        return;
      }
    ...
    this.events.click = event => {
      const state = this.getState();
      const dragOffsetX =
        this.props.direction !== "vertical" ? state.dragOffset.x : 0;
      const dragOffsetY =
        this.props.direction !== "horizontal" ? state.dragOffset.y : 0;
      if (
        Math.max(Math.abs(dragOffsetX), Math.abs(dragOffsetY)) >
        CLICK_EVENT_THRESHOLD_PX
      ) {
        event.preventDefault();
